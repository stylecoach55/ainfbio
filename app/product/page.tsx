...
export default function ProductPage(){return <main className="product-page">
  ...
  <section className="phase-section section"><div className="wrap"><div className="section-intro row"><div><p className="eyebrow">02 · INGREDIENT STORY</p><h2>피부에 닿는 4단계 성분 설계</h2></div></div><div className="phase-grid">{phases.map((item,index)=><article className={`phase-card phase-card--${index+1}`} key={item.no}><span>{item.no}</span><h3>{item.title}</h3><div className="phase-motif" aria-hidden="true"><i /><i /><i /></div><ul>{item.ingredients.map(ingredient=><li key={ingredient}>{ingredient}</li>)}</ul></article>)}</div><div className="ion-story"><div className="ion-story-head"><p className="eyebrow">QUANTUM ENERGY CONCEPT</p><h3><span>퀀텀 에너지와 함께</span><span>피부를 위한 4가지 이온 모티브</span></h3><p>큐티 이지케어만의 차별화된 에너지 컨셉</p></div><div className="ion-elements">{ionElements.map(item=><article key={item.symbol}><strong>{item.symbol}</strong><span>{item.en}</span><b>{item.ko}</b></article>)}</div></div></div></section>
  ...
  <section className="moment-section section"><div className="wrap"><div className="section-intro row"><div><p className="eyebrow">04 · EVERYDAY, EVERYWHERE</p><h2 className="routine-title"><span>간편한</span><strong>1-Minute Skincare</strong></h2></div></div><div className="moment-grid moment-grid-lifestyle">{moments.map(item=><article key={item.label}><img className="moment-image" src={item.image} alt={item.alt} loading="lazy" /><span className="ai-image-label">AI로 생성된 가상인물 이미지</span><div className="moment-content"><span>{item.label}</span><h3>{item.title}</h3>{item.text && <p>{item.text}</p>}</div></article>)}</div></div></section>
