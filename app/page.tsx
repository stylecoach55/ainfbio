...
export default function Home() {
  return <main>
    ...
    <section className="home-product section" id="product"><div className="wrap home-product-grid"><div className="home-product-image"><img src="/assets/qt-easycare-pouch-hd.png" alt="QT easycare 파우치와 코튼 패드" loading="lazy" /></div><div className="home-product-copy"><p className="eyebrow">02 · SIGNATURE PRODUCT</p><h2><ProductName /></h2><p className="home-product-tag">1-Minute Skincare</p><p className="home-product-lead">에센스를 머금은 코튼 패드 타입으로, 개별 파우치 포장되어 간편하게 사용할 수 있습니다.</p><div className="brief-features">{briefFeatures.map(item => <article key={item.no}><span>{item.no}</span><div><h3>{item.title}</h3><p>{item.text}</p></div></article>)}</div><div className="button-row"><a className="button button-primary" href="/product">제품 상세페이지 보기</a><a className="button button-outline" href={STORE_URL} target="_blank" rel="noopener noreferrer">네이버 스마트스토어에서 구매하기</a></div></div></div></section>
